...
class GlobalExceptionHandler(
    ...
    fun onException(exception: Exception, locale: Locale): ResponseEntity<ExceptionDto> {
        exception.printStackTrace()
        return exceptionResponseBuilder.build(exception, INTERNAL_SERVER_ERROR, locale)
    }

    @ExceptionHandler(MethodArgumentNotValidException::class)
    fun onValidationException(exception: MethodArgumentNotValidException, locale: Locale): ResponseEntity<ExceptionDto> {
