...
/*
    Another way to handle coroutine cancellation or completion is to use the
    invokeOnCompletion function from Job, which is used to set a handler to be
    called when the job reaches a terminal state, namely either “Completed” or
    “Cancelled”. It also provides an exception in its parameter that finishes a
    coroutine. invokeOnCompletion is guaranteed to be called exactly once (assuming
    this coroutine ever completes), even if the job had already completed when the
    handler was set. invokeOnCompletion is called synchronously during cancellation, and we can’t
    control the thread in which it will be running. I
...
/*
    Cancellation propagates down the hierarchy of coroutines because when a job is
    cancelled, all its children are also cancelled. This is a very useful feature as when
    you cancel a process, it also cancels all its subprocesses. This is good practice as it
    prevents memory leaks and frees resources.

    Once a job is cancelled it cannot be used as a parent for new coroutines. So, a scope with a
    cancelled job is not useful anymore. This can even be dangerous because trying to start a new
    coroutine in such a scope will silently do nothing.

    That is why Its recommend using the cancelChildren function from
    CoroutineContext, which cancels all the children of a job but leaves the job itself in the active state.
*/
...
/*
    What if cleaning up resources requires making suspending calls or starting coroutines? We cannot do that because
    these operations are not allowed in the “Cancelling” state. Calling suspending functions in this state will throw
    CancellationException, and starting a new   coroutine will be ignored.
    For such situations, there is a special structure withContext(NonCancellable).We should use withContext(NonCancellable)
    for all suspending calls that should be executed even in the “Cancelling” state (so even in the case of cancellation).
    NonCancellable is a job that is always active, and it should not be used outside this
    particular situation, where we need to either make a suspending call or start a new coroutine even in the case of cancellation.
*/
...
/*
    Even if you just implement a suspending function and you specify some cleanup in the finally block that requires
    a suspending call, you should use withContext(NonCancellable) to make sure that the cleanup will be done even
...
/*
    ...
    To start a new coroutine ormake a suspending call in the “Cancelling” state, we can use withContext(NonCancellable).
    ...
    Always use suspendCancellableCoroutine instead of suspendCoroutine when you need to transform a callback-based API into a suspending
    function, and use invokeOnCancellation to define what should happen
